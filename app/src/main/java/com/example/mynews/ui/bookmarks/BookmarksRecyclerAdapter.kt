...
class BookmarksRecyclerAdapter(private var articles: MutableList<Article>, private val clickListener: ClickListener) :
    ...
    inner class ImageViewHolder(private val binding: ItemNewsWithImageBinding) :
        ...
        fun bind() {
            ...
            binding.tvTitle.text = model.title
            binding.tvDescription.text = model.description
            Glide.with(binding.ibNewsImage.context).load(model.urlToImage)
                .into(binding.ibNewsImage)
            binding.bookmarkSign.setOnClickListener {
                clickListener.onBookmarkClick(model)
            }
            binding.tvTitle.setOnClickListener {
